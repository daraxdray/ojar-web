                    ...
                    <div class="flex items-center justify-center w-full p-6 sm:p-12 ">
                        <div class="w-full">
                            ...
                            <div x-data="{ tab: window.location.hash ? window.location.hash.substring(1) : 'driver' }" id="tab_wrapper">
                                ...
                                <nav class="mb-8">
                                    <a :class="tab==='driver' ? 'bg-primary-500 text-white' : 'bg-primary-200 text-gray-500'" class="p-2 font-semibold text-white rounded-t-lg text-md" @click.prevent="tab = 'driver'; window.location.hash = 'driver'" href="#">{{ __('Driver') }}</a>
                                    <a :class="tab==='vendor' ? 'bg-primary-500 text-white' : 'bg-primary-200 text-gray-500'" class="p-2 font-semibold text-white rounded-t-lg text-md" @click.prevent="tab = 'vendor'; window.location.hash = 'vendor'" href="#">{{ __('Vendor') }}</a>
                                </nav>
                                ...
                                <div x-show="tab === 'driver'">
                                    <form wire:submit.prevent="driverSignUp">
                                        ...
                                        <x-input title="{{ __('Phone(country code)*') }}" name="phone" placeholder="+2335575..." />
                                        ...
                                        <hr class="my-4" />
                                        <p class="font-light">{{ __('Documents(ID Cards & Guarantor\'s Form)') }}</p>
                                        <livewire:component.multiple-media-upload title="{{  __('Documents') }}" types="PNG or JPEG" fileTypes="image/*" emitFunction="driverDocumentsUploaded" />
                                ...
                                <div x-show="tab === 'vendor'">
                                    <form wire:submit.prevent="vendorSignUp">
                                        ...
                                        <livewire:component.multiple-media-upload title="{{ setting('page.settings.vendorDocumentInstructions', __('Documents')) }}" types="PNG or JPEG" fileTypes="image/*" emitFunction="vendorDocumentsUploaded" />
