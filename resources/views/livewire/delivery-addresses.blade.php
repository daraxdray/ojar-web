...
<div>
    ...
    <x-baseview title="{{ __('Delivery Address') }}">
    ...
    <div x-data="{ open: @entangle('showCreate') }">
        <x-modal confirmText="{{ __('Save') }}" action="save">
            <p class="text-xl font-semibold">{{ __('New Delivery Address3') }}</p>
            @if(!Auth::user()->hasRole('client'))
            <x-autocomplete-input title="{{ __('User') }}" name="user" :dataList="$users ?? []" emitFunction="autocompleteUserSelected" />
            @else
            <x-input wire:model="user_id" name="user" value="{{Auth::user()->name}}"  type="hidden"/>
            @endif
